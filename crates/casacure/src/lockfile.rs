...
fn enolck_note() {
    ...
    if !NOTED.swap(true, Ordering::Relaxed) {
        eprintln!("LockFile: locks not supported on this filesystem (ENOLCK); proceeding without");
    }
}
...
impl FileLocker {
    ...
    /// someone else (`EAGAIN`/`EACCES`). `Err` = hard error (incl. `EBADF`
    /// for a write lock on a read-only fd).
    ...
    /// Whether an exclusive lock could be taken right now (used both for the
    /// byte-1 "in use" probe and for read-lock holders to detect a
...
impl TableSyncData {
    ...
    /// the huge-nrrow v2 form above `u32::MAX` like `TableSyncData::write`).
...
impl LockFile {
    ...
    /// `permLocking` map onto the use-locker width). Returns `Ok(None)` for
    /// `NoLocking`.
    pub fn attach(
        dir: &Path,
    ...
    pub fn is_multi_used(&self) -> io::Result<bool> {
        if self.missing || self.read_only_fd {
            // Without a writable fd the probe itself cannot run; report the
            // honest "not observable" answer casacore's no-locking mode gives.
            return Ok(false);
        }
        Ok(!self.use_locker.can_write_lock()?)
    }

    /// `LockFile::acquire` (+ `addReqId` while retrying). `nattempts == 0`
    /// blocks. `Ok(false)` = gave up after `nattempts`.
    ///
    ...
    /// Release a *read* hold. No-op while a write lock is held — it covers
    /// the read case, and unlocking the byte would drop the writer's
    /// exclusion for the whole process.
    ...
    /// info when given (the `TableLockData::release` callback). No-op when
    ...
    /// `TableLockData::autoRelease` + `LockFile::inspect`: throttled check
    /// (every 25th call and at most once per interval) for a waiting
    /// process in the request list.
    ...
    /// stream. Missing/short info (a just-created or foreign file) -> `None`.
    ...
    /// `LockFile::putInfo`: length-prefixed info at `SIZE_REQ_ID` + fsync.
    /// A no-op for a missing lock file (copied tables operate lock-free,
    /// exactly like the retired `patch_lock_nrrow` no-op contract) and for
    /// a read-only fd.
...
fn can_write(f: &File) -> bool {
    // The only portable way to know the fd is writable is to try: probe an
    // fcntl write lock over a byte we do not otherwise use (the tail of the
    // request list area is file data, so probe far beyond any real file —
    // fcntl locks work past EOF). Byte at offset 1<<20.
    matches!(
...
fn no_underlying_file() -> io::Result<File> {
    // A placeholder for the missing-file case; never used for I/O because
    // every path checks `missing` first.
    File::open("/dev/null")
}
...
/// One `table.lock` per path per process, shared by every handle: the fd
/// must stay open while *any* lock is held, and POSIX drops the process's
/// locks the moment any fd to the file is closed. Handles therefore share
/// one fd through this registry; the fd (and its locks) live until the
/// last referencing handle drops.
static LOCK_REGISTRY: OnceLock<Mutex<std::collections::HashMap<PathBuf, Weak<Mutex<LockFile>>>>> =
    OnceLock::new();

pub type SharedLockFile = Arc<Mutex<LockFile>>;

/// The process's shared instance for a directory's `table.lock`, when one
/// exists. Callers about to open `table.lock` transiently must check here
/// first: POSIX drops a process's record locks when *any* fd to the file
/// closes, so with a live instance every read must go through its fd.
