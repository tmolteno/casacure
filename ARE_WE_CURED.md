...
| §2 Table lifecycle & locking | DONE | `Table` open/create, lock/unlock, flush/close, iswritable/name, nrows/colnames; Send+Sync; casacore locking protocol on `table.lock` (fcntl, lockoptions, sync record) verified against real python-casacore both ways |
