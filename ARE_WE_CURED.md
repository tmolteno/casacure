...
| §8 dask-ms integration / bindings | ~80% | `casacure-python` pyo3 binding with exact casacore array semantics; **dask-ms runs on casacure**: xds_from_table/xds_to_table read+write and group_cols GROUPBY partitioning verified via a `casacore.tables` shim (`tests/daskms_smoke.py`), cross-checked with real casacore; remaining: broader dask-ms fixtures + upstream store dispatch |
