...
| §8 dask-ms integration / bindings | ~88% | pyo3 binding + **full dask-ms MS lifecycle on casacure**: xds_from_table/xds_to_table read+write, group_cols GROUPBY partitioning, and xds_to_ms/xds_from_ms create/write/read of a full MS (default_ms context manager, 12 subtables) via a `casacore.tables` shim (`tests/daskms_smoke.py`), cross-checked with real casacore; remaining: broader fixtures (tablefromascii/apps) + upstream store dispatch |
