...
|---|---|---|---|
| Rust unit + fixture tests | `cargo test` | 72/72 | type system, AipsIO read+write (both endians), `table.dat`, StandardStMan data file + `table.f0i` + string buckets, IncrementalStMan (interval index, multi-DM tables) — all read+write |
...
| §5 Metadata & descriptors | ~60% | read + write done: nrows/colnames/getcoldesc/getdesc, getkeywords/getcolkeywords, putkeyword/putcolkeyword/removekeyword/removecolkeyword (nested records); subtable linkage pending |
